...
export class LotesController {
  ...
  @Permissions("escritura:lote")
  @ApiOperation({
    summary: "Enviar un animal a enfermería",
    description:
      "Marca la excepción `id_corral_enfermeria` (primer corral 'enfermeria' " +
      "activo de la empresa, o el indicado si hay varias). La ubicación común " +
      "del animal sigue siendo la de su lote.",
  })
  @ApiParam({ name: "id", type: Number, description: "ID del lote" })
  @ApiParam({ name: "animalId", type: Number, description: "ID del animal" })
  enviarEnfermeria(
    ...
    return this.lotesService.enviarEnfermeria(
      id,
      animalId,
      dto?.idCorral,
      req.user,
  ...
  @ApiOperation({
    ...
    description:
      "Limpia `id_corral_enfermeria`: vuelve al corral ACTUAL de su lote (por derivación).",
  })
  @ApiParam({ name: "id", type: Number, description: "ID del lote" })
  @ApiParam({ name: "animalId", type: Number, description: "ID del animal" })
  traerDeEnfermeria(
    ...
    return this.lotesService.traerDeEnfermeria(id, animalId, req.user);
  }
}
